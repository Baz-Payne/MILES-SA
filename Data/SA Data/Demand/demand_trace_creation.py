...
# Save demand trace
total_demand_no_hydrogen.to_csv('Intermediate/OperationalDemandNoH20.csv', index=False)
...
for region, split, file in region_splits[['Honours Region', 'Splits', 'Save File']].itertuples(index=False):
    ...
    if "NoH20" in file:
        regional_demand.to_csv('Intermediate/' + file, index=False)
    else:
        regional_demand.to_csv('Multi Nodal/' + file, index=False)