# Islanded Copper Plate Grid Model k Factor Investigation
...
# Define the different q values that will be investigated
q_values = list(np.arange(-1, -13, -1))
q_values = [10.0**exp for exp in q_values]
q_values.insert(0, 0)

# Define the save file name for this investigation
save_name = 'Results/q Investigations (Full).xlsx'
...
# Investigate a single net renewable year
year = 2047
...
for q in q_values:
    ...
    # Define start and end dates for the chosen net renewable year
    ...
    mng += pulp.lpSum(C_e * E_max[r] + C_p * P_max[r] for r in sa_regions) - k * pulp.lpSum(E[r][t] for r in sa_regions for t in range(T)) + q * delta_t * pulp.lpSum(P_curt[r][t] for r in sa_regions for t in range(T))
    ...
    # Problem Solve time
    problem_solve_time.loc[0, q] = elapsed_time
    problem_solve_time.loc[0, 'Type'] = "Solve Time (s)"
    
    # Calculate total storage requirement
    annual_storage.loc['Total Storage', q] = total_storage
    ...
    for n in all_regions:
        ...
        # Otherwise its an interstate region
            ...
            if n == 'Murraylink':
                ...
                annual_import.loc[n, q] = total_imports
                annual_import.loc[n, 'Type'] = n + " Annual Imports (TWh)"
                
                annual_export.loc[n, q] = total_exports
    ...
    # Combine the annual output totals
