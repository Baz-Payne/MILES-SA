### Multi Nodal Grid Sensitivity Studies ###
...
### Select Sensitivty to Run ###

# Set only one to true
daily_interconnector_restriction = False
dispatchable_generation = False
renewable_oversize = True

selected = [daily_interconnector_restriction, dispatchable_generation, renewable_oversize]
if sum(selected) != 1:
    raise ValueError("Please select a single sensitivity analysis to run at a time.")

# Define the daily amount of interconnector use
daily_interconnector_use = 0
interconnector_hourly_equivalent = 6   # This variable is in hours of the day

# Define dispatchable generator size
dispatchable_percent = 0.03     # Dispatchable generation as a percentage of total demand
disp_gen_size = 0.6             # Dispatchable generation size in GW

# Define renewable energy oversize
renewable_oversize_factor = 1.05    # Percentage increase above AEMO's baseline

# Setup save file
save_location = ""
if daily_interconnector_restriction:
    save_location = "Sensitivity Results/Interconnector/" + str(interconnector_hourly_equivalent) + " hours"
elif dispatchable_generation:
    save_location = "Sensitivity Results/Dispatchable Generator/" + str(int(disp_gen_size*1000)) + "MW " + str(int(dispatchable_percent*100)) + " percent"
else:
    save_location = "Sensitivity Results/Renewable Oversize/" + str(int(renewable_oversize_factor*100)) + " percent"
...
for region in sa_regions:
    ...
    solar_generation[region].iloc[:,1] = solar_generation[region].iloc[:,1] / 10**3 * (renewable_oversize_factor if renewable_oversize else 1)
    ...
    wind_generation[region].iloc[:,1] = wind_generation[region].iloc[:,1] / 10**3 * (renewable_oversize_factor if renewable_oversize else 1)
...
# Define daily interconnector restriction
if daily_interconnector_restriction == True:
...
for year in investigation_period:
    ...
    if dispatchable_generation:
        P_disp = pulp.LpVariable.dicts("P_disp", range(T), lowBound=0, upBound=disp_gen_size)
        ...
        for t in range(T):
            ...
            if n == sa_regions[0]:
                if t % hours_in_day == 0:
                    ...
                    # Add restriction on daily interconnector use
                    if daily_interconnector_restriction:
                        mng += pulp.lpSum(P_I[s, r, d] for s in interstate_regions for r in sa_regions if transmission_matrix.loc[s, r] > 0 for d in range((day-1)*hours_in_day,day*hours_in_day))*delta_t <= daily_interconnector_use
    ...
    if daily_interconnector_restriction:
        ...
        mng += delta_t * pulp.lpSum(P_I[s, r, t] for s in interstate_regions for r in sa_regions if transmission_matrix.loc[s, r] > 0 for t in range(T)) <= annual_interconnector_use.loc[0,year] * 10**3
    ...
    for n in all_regions:
        ...
        if n in sa_regions:
            for b in storage_types:
                CY_storage_traces[n][b].to_csv(save_location + f' {year}_{n}_{b}_Traces.csv', index=False)
            
        CY_traces[n].to_csv(save_location + f' {year}_{n}_Traces.csv', index=False)
        transmission_flow[n].to_csv(save_location + f' {year}_{n}_Xmission_Traces.csv', index=False)
            
    
# Save the annual input and output totals
with pd.ExcelWriter(save_location + ' Annual Totals.xlsx') as writer:
