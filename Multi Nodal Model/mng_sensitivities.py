...
### Multi Nodal Grid Transmission Line Variance Sensitivity ###
...
# Define the growth in the current transmission assets
transmission_growth = 1.5
...
for year in investigation_period:
    ...
    annual_demand_total = sum(D[n].iloc[t, 1] for n in sa_regions for t in range(T)) * delta_t 
    USE_limit = annual_demand_total * USE_percent
    ...
    cost = sum(C_e[b] * 1e9 * pulp.value(E_max[r][b]) + C_p[b] * 1e9 * pulp.value(P_max[r][b]) for r in sa_regions for b in storage_types) / 10**6
    print("Estimated system cost: ${:,.0f} million".format(cost))
    ...
    for n in all_regions:
        ...
        if n in sa_regions:
            for b in storage_types:
                CY_storage_traces[n][b].to_csv(f'Sensitivity Results/Transmission/{year}_{n}_{b}_Traces.csv', index=False)
            
        CY_traces[n].to_csv(f'Sensitivity Results/Transmission/{year}_{n}_Traces.csv', index=False)
        transmission_flow[n].to_csv(f'Sensitivity Results/{year}_{n}_Xmission_Traces.csv', index=False)
            
    
# Save the annual input and output totals
with pd.ExcelWriter('Sensitivity Results/Transmission/Annual Totals Transmission.xlsx') as writer:
...
### Multi Nodal Grid Interstate Reliance ###
...
sheet_name_DSP = 'DSP'
sheet_name_inter_demand = 'Interstate Net Demand'
sheet_name_interstate_imports = 'Imports and Exports'
...
# Obtain AEMO's forecast imports and export for SA
annual_interconnector_use = pd.read_excel(file_path, sheet_name=sheet_name_interstate_imports)
...
# Define the daily amount of interconnector use
daily_interconnector_use = 0
interconnector_hourly_equivalent = 12

for s in interstate_regions:
    for r in sa_regions:
        
        # Limit the total daily interconnector use to a maximum of x hours continuous use
        daily_interconnector_use += transmission_matrix.loc[s,r] * interconnector_hourly_equivalent
...
for year in investigation_period:
    ...
    for n in all_regions:
        ...
        for t in range(T):
            ...
            # Add constraint for daily amount of DSP among all regions and interconnector use
            if n == sa_regions[0]:
                if t % hours_in_day == 0:
                    mng += pulp.lpSum(P_DSP[r][d] for r in sa_regions for d in range((day-1)*hours_in_day,day*hours_in_day))*delta_t <= daily_DSP_limit
                    mng += pulp.lpSum(P_I[s, r, d] for s in interstate_regions for r in sa_regions for d in range((day-1)*hours_in_day,day*hours_in_day))*delta_t <= daily_interconnector_use
                    day += 1
    ...
    # Limit the annual amount of interconnector use such that it is aligned with AEMO's
    mng += delta_t * pulp.lpSum(P_I[s, r, t] for s in interstate_regions for r in sa_regions for t in range(T)) <= annual_interconnector_use.loc[0,year] * 10**3
    ...
    for n in all_regions:
        ...
        if n in sa_regions:
            for b in storage_types:
                CY_storage_traces[n][b].to_csv(f'Sensitivity Results/Interconnector/{year}_{n}_{b}_Traces.csv', index=False)
            
        CY_traces[n].to_csv(f'Sensitivity Results/Interconnector/{year}_{n}_Traces.csv', index=False)
        transmission_flow[n].to_csv(f'Sensitivity Results/{year}_{n}_Xmission_Traces.csv', index=False)
            
    
# Save the annual input and output totals
with pd.ExcelWriter('Sensitivity Results/Interconnector/Annual Totals 12h.xlsx') as writer:
    annual_input_totals.to_excel(writer, sheet_name='Annual Input Totals', index=False)
    annual_output_totals.to_excel(writer, sheet_name='Annual Output Totals', index=False)
...
### Multi Nodal Grid Dispatchable Generator Sensitivity ###
...
annual_import = pd.DataFrame(columns=column_headings)
annual_export = pd.DataFrame(columns=column_headings)
annual_dispatch = pd.DataFrame(columns=column_headings)
...
# Define the percentage of dispatchable generation
dispatchable_percent = 0.02
...
for year in investigation_period:
    ...
    P_DSP = {r: pulp.LpVariable.dicts(f"P_DSP_{r}", range(T), lowBound=0, upBound=DSP_capacity) for r in sa_regions}
    USE = {r: pulp.LpVariable.dicts(f"USE_{r}", range(T), lowBound=0, upBound=0.3) for r in sa_regions}
    P_disp = pulp.LpVariable.dicts("P_disp", range(T), lowBound=0, upBound=0.35)
    
    # Currently running unlimited usage of dispatchable plant that is 350MW in size. 252 GWh result
    # Next, running 1% usage of dispatchable plant that is 350MW in size. 304.4 GWh result
    # Next, running 2% usage of dispatchable plant that is 350MW in size. 252.9 GWh result
    # Next, running 3% usage of dispatchable plant that is 350MW in size. GWh result
        ...
        for t in range(T):
            ...
            if n in sa_regions:
                ...
                if n == 'Adelaide Metro':
                    ...
                    # Supply and demand balance constraint for Adelaide metro region with dispatchable generator
                    mng += (
                        G[n].iloc[t, 1]
                        + pulp.lpSum(P_disc[n][b][t] for b in storage_types)
                        + pulp.lpSum(P_I[j, n, t] for j in all_regions if j != n) # incoming transmission
    ...
    # Limit the amount of dispatchable generation to a small percentage of total demand
    mng += delta_t * pulp.lpSum(P_disp[t] for t in range(T)) <= annual_demand_total * dispatchable_percent
    ...
    # Total dispatchable generation used
    annual_dispatch.loc[0, year] = sum(pulp.value(P_disp[t]) for t in range(T)) / 10**3
    annual_dispatch.loc[0, 'Type'] = 'Dispatchable Generation (TWh)'
    ...
    annual_output_totals = pd.concat([annual_storage, total_annual_storage, annual_power, annual_curtailment,
                                      annual_charge, annual_discharge, annual_equivalent_cyles,
                                      annual_SoC_percentage, annual_import, annual_export, annual_dispatch, problem_solve_time, system_cost],ignore_index=True)
    ...
    for n in all_regions:
        ...
        if n in sa_regions:
            for b in storage_types:
                CY_storage_traces[n][b].to_csv(f'Sensitivity Results/Dispatch/{year}_{n}_{b}_Traces.csv', index=False)
            
        CY_traces[n].to_csv(f'Sensitivity Results/Dispatch/{year}_{n}_Traces.csv', index=False)
        transmission_flow[n].to_csv(f'Sensitivity Results/Dispatch/{year}_{n}_Xmission_Traces.csv', index=False)
            
    
# Save the annual input and output totals
with pd.ExcelWriter('Sensitivity Results/Dispatch/Annual Totals Dispatchable 2 Percent.xlsx') as writer:
    annual_input_totals.to_excel(writer, sheet_name='Annual Input Totals', index=False)
    annual_output_totals.to_excel(writer, sheet_name='Annual Output Totals', index=False)
...
'''  
### Multi Nodal Grid More Renewable Sensitivity ###
...
# Increase the total amount of renewable energy above AEMO's
renewable_increase_factor = 1.01
...
for region in sa_regions:
    ...
    solar_generation[region] = pd.read_csv(generation_dir + mapping_table.loc[mapping_table['Honours Region'] == region, 'Solar Generation File'].iloc[0])
    solar_generation[region]['Datetime'] = pd.to_datetime(solar_generation[region]['Datetime'])
    solar_generation[region].iloc[:,1] = solar_generation[region].iloc[:,1] / 10**3 * renewable_increase_factor
    
    # Read in the wind generation of the current SA region
    wind_generation[region] = pd.read_csv(generation_dir + mapping_table.loc[mapping_table['Honours Region'] == region, 'Wind Generation File'].iloc[0])
    wind_generation[region]['Datetime'] = pd.to_datetime(wind_generation[region]['Datetime'])
    wind_generation[region].iloc[:,1] = wind_generation[region].iloc[:,1] / 10**3 * renewable_increase_factor
...
for year in investigation_period:
    ...
    for n in all_regions:
        ...
        if n in sa_regions:
            for b in storage_types:
                CY_storage_traces[n][b].to_csv(f'Sensitivity Results/{year}_{n}_{b}_Traces.csv', index=False)
            
        CY_traces[n].to_csv(f'Sensitivity Results/{year}_{n}_Traces.csv', index=False)
        transmission_flow[n].to_csv(f'Sensitivity Results/{year}_{n}_Xmission_Traces.csv', index=False)
            
    
# Save the annual input and output totals
with pd.ExcelWriter('Sensitivity Results/Annual Totals.xlsx') as writer:
    annual_input_totals.to_excel(writer, sheet_name='Annual Input Totals', index=False)
    annual_output_totals.to_excel(writer, sheet_name='Annual Output Totals', index=False)
